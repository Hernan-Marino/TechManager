...
class VentanaEquipos(QWidget):
    ...
    def crear_botones_acciones(self, equipo):
        ...
        boton_detalle.clicked.connect(lambda: self.ver_detalle_equipo(equipo['id_equipo']))
        layout.addWidget(boton_detalle)
        
        # Botón Cambiar Estado
        boton_estado = Boton("🔄 Estado", "secundario")
        boton_estado.setMaximumWidth(90)
        boton_estado.clicked.connect(lambda: self.cambiar_estado(equipo['id_equipo']))
        layout.addWidget(boton_estado)
        
        # Botón Remito
        boton_remito = Boton("📋", "neutro")
        boton_remito.setToolTip("Ver remito")
        boton_remito.setMaximumWidth(50)
        boton_remito.clicked.connect(lambda: self.ver_remito(equipo['id_equipo']))
        layout.addWidget(boton_remito)
